...
class DataClassGenerator {
  ...
  insertFromMap(clazz) {
    ...
    for (let p of props) {
      ...
      if (p.isEnum) {
        if (p.isCollection) {
          method += `(${value} as List).isEmpty ? [] :  ${
            p.type
          }.from(${value}.map((x) => ${p.type
            .replace("List<", "")
            .replace(">", "")}.values.byName(x)))`;
        } else {
          method += `${p.type
            .replace("List<", "")
            .replace(">", "")}.values.byName(${value})`;
        }
      } else if (p.isCollection) {
        method += `${p.type}.from(`;
        if (p.isPrimitive) {
        ...
        method += `${value}${
          p.isDouble ? "?.toDouble()" : p.isInt ? "?.toInt()" : ""
        }`;
      } else {
        method += customTypeMapping(p);
      }
